...
void CCommandStrDlg::OnOK() 
{
	...
	Command _command;
	memset(&_command,0,sizeof(Command));//初始化命令行结构体
	bool bRet = SetCommand(strCommand.GetBuffer(strCommand.GetLength()),_command);//设置命令属性,并返回是否设置成功
	if(bRet == false)
	{
		AfxMessageBox("命令行读取失败，请检查命令行!");
		return;
	}
	OutPrintf(_command); //打印输出
}
	...
	{//根据输入的命令设置命令属性
		...
		else if(strcmp(_str.c_str(),"-o") == 0)
		{
			...
			if(strcmp(_str.c_str(),"-c") == 0 ||
				...
				_str.empty())
				return false;
			strcpy(command.outFile , _str.c_str());//打开的文件只对当前路径有效
		}
		...
		else if(strcmp(_str.c_str(),"-e") == 0)
		{
			...
			if(strcmp(_str.c_str(),"-c") == 0 ||
				...
				_str.empty())
				return false;
			strcpy(command.StopString,_str.c_str());//打开的文件只对当前路径有效
		}
		else if(strcmp(_str.c_str(),"-a") == 0)
			command._a = true;
		else if(!_str.empty())
			strcpy(command.filePath,_str.c_str()); //打开的文件只对当前路径有效
		else
			return false;		
	}
	if(bIsEmpty)
...
void CCommandStrDlg::OpenMyFile(CString strPath,Command command)
{
	...
	{
		CString str;
		while(myFile.ReadString(str)) //逐行读取
		{
			istringstream iss;
...
void CCommandStrDlg::ShowToWindow(int nCount,int nString,int nChar,Command command)
{
	...
	if(myFile.Open(strPath,CFile::modeCreate | CFile::modeReadWrite))
	{
		...
		if(command._c)
		{
			strFormat.Format("%s,字符数:%d",command.filePath,nChar);
		...
		if(command._w)
		{
			strFormat.Format("%s,单词数:%d",command.filePath,nString);
			m_List.AddString(strFormat);
			strFormat = strFormat + "\n";
			myFile.WriteString(strFormat);
		}
		...
		if(command._l)
		{
			strFormat.Format("%s,行数:%d",command.filePath,nCount);
			m_List.AddString(strFormat);
			strFormat = strFormat + "\n";
			myFile.WriteString(strFormat);
		}
